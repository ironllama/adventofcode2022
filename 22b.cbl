      ...
      perform varying move_idx from 1 by 1 until move_idx > dir_num or dir_blocked = 1
        ...
        evaluate test_facing
          ...
          when 2
            ...
            *>   if test_x = 0 move rf_chars_in_row to test_x end-if
            *> end-perform
            subtract 1 from test_x
          when 3
            *> perform with test after until rf_row(test_y)(test_x:1) <> space
            *>   compute test_y = function mod((test_y - 1) rf_chars_in_col)
            *>   if test_y = 0 move rf_chars_in_col to test_y end-if
            *> end-perform
            subtract 1 from test_y
        end-evaluate
        ...
        *> If moved out of range, teleport to proper place within range.
        evaluate test_facing
          when 0
            *> 2 to 4
            if test_x = 151 and test_y >=1 and test_y <= 50
              ...
              compute curr_facing = function mod((curr_facing + 2) 4)
            end-if
            *> 3 to 2
            if test_x = 101 and test_y >= 51 and test_y <= 100
              ...
              compute curr_facing = function mod((curr_facing - 1) 4)
            end-if
            *> 4 to 2
          ...
          when 3
            ...
            *> 5 to 3
            if test_y = 150 and test_x >= 1 and test_x <= 50
              compute test_y = test_x + 50
              compute test_x = 51
              compute test_facing = function mod((test_facing + 1) 4)
              compute curr_facing = function mod((curr_facing + 1) 4)
